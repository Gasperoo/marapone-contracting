...
export const PRODUCTS = {
  ...
  // The two-product "pair" was retired when ScopeGuard shipped: the whole
  // argument for a bundle is that the job carries through without being retyped,
  // and that argument only completes at buyout. `pair` is kept as an alias so a
  // checkout link posted before the change still resolves — it quotes the suite.
  suite: {
    label: 'Blueprint Auditor + AI Estimator + ScopeGuard',
    price: 2490,
    description: 'All three products, wired together so one job runs from the drawing set to the awarded subcontract with nothing retyped. Includes a setup call on a real job of yours. One-time purchase, full source code, no subscription.',
  },
};
