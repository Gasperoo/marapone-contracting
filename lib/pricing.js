 ...
 */
const PACK_TAIL = 'One-time purchase, no subscription. 13% HST added at checkout.';

export const DATA_PACKS = {
  'pack-code-library': {
    label: 'Building Code Compliance Library',
    price: 89,
    description: `The code-check ruleset behind Blueprint Auditor, as PDF + CSV, one folder per jurisdiction — every Canadian province and territory, all 50 US states, and every EU country plus the UK and Ireland. ${PACK_TAIL}`,
  },
  'pack-assembly-schema': {
    label: 'Assembly Property Schema',
    price: 89,
    description: `The structured assembly data behind SpecChecker, as JSON + CSV — CSI MasterFormat-aligned assemblies for North America and EN/Eurocode-aligned assemblies for Europe, kept separate rather than blended. ${PACK_TAIL}`,
  },
  'pack-rate-2026': {
    label: 'Estimating Rate Pack — 2026 Edition',
    price: 99,
    description: `Permits, development charges, GST/HST/PST, US state sales tax and EU VAT by jurisdiction, plus the contingency-risk framework behind the AI Estimator. Dated edition — next year's rates ship as a new edition, not a silent update. ${PACK_TAIL}`,
  },
  'pack-bid-leveling': {
    label: 'Bid-Leveling Template',
    price: 59,
    description: `The scope-element checklist and blank comparison matrix behind the Bid Leveler, with a region-notes tab covering where North American and European tendering conventions differ. ${PACK_TAIL}`,
  },
  'pack-scope-checklist': {
    label: 'Master Scope Requirement Checklist',
    price: 59,
    description: `The master scope-requirement library behind ScopeGuard — requirements across 20 trade packages, each carrying its CSI code — with the same region-notes layer. ${PACK_TAIL}`,
  },
  'pack-bundle': {
    label: 'The Full Data Pack Bundle — all five libraries',
    price: 349,
    description: `All five reference libraries — code compliance, assembly schema, estimating rates, bid levelling and scope requirements — covering Canada, the US and the EU/UK in one download. Saves $46 against buying them separately. ${PACK_TAIL}`,
  },
};
