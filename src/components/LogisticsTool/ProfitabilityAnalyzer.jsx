...
import { analyzeShipmentProfitability, generatePerformanceScorecard, trackQuoteEngagement, getConversionMetrics } from '../GasperTool/services/analyticsService';
...
export function ProfitabilityAnalyzer() {
    ...
    return (
        <div className="space-y-6">
            {/* View Selector */}
            <div className="flex gap-2 p-1 bg-black/[0.03] rounded-lg w-fit">
                <button
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${view === 'profitability' ? 'bg-glass-highlight text-white' : 'text-gray-400 hover:text-[#1a1a1a] hover:bg-black/[0.03]'}`}
                    onClick={() => setView('profitability')}
                >
                    Profitability
                </button>
                <button
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${view === 'performance' ? 'bg-glass-highlight text-white' : 'text-gray-400 hover:text-[#1a1a1a] hover:bg-black/[0.03]'}`}
                    onClick={() => setView('performance')}
                >
                    Performance
                </button>
                <button
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${view === 'quotes' ? 'bg-glass-highlight text-white' : 'text-gray-400 hover:text-[#1a1a1a] hover:bg-black/[0.03]'}`}
                ...
                <div className="space-y-4">
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xl font-semibold text-[#1a1a1a]">Shipment Cost Analysis</h3>
                            <div className="w-48">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div className="p-4 bg-blue-500/10 rounded-lg">
                                ...
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatCurrency(profitability.quoted.total)}</p>
                            </div>
                            <div className="p-4 bg-orange-500/10 rounded-lg">
                                <p className="text-sm text-orange-200">Actual Total</p>
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatCurrency(profitability.actual.total)}</p>
                            </div>
                            ...
                            <div className="p-4 bg-green-500/10 rounded-lg">
                                ...
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatPercentage(profitability.profitability.margin)}</p>
                            </div>
                        </div>

                        <div className="space-y-3">
                            <h4 className="font-semibold text-[#1a1a1a]">Cost Breakdown</h4>
                            {['freight', 'duties', 'taxes', 'fees'].map((category) => (
                                <div key={category} className="flex items-center justify-between p-3 bg-black/[0.03] rounded border border-black/[0.04] hover:bg-black/[0.06] transition-colors">
                                    <span className="capitalize font-medium text-[#1a1a1a]">{category}</span>
                    ...
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <h4 className="font-semibold mb-4 text-[#1a1a1a]">Cost Trend Analysis</h4>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={costTrendData}>
                                    ...
                                    <Tooltip
                                        contentStyle={{ backgroundColor: 'rgba(23, 23, 23, 0.9)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '8px', color: '#fff' }}
                                    />
            ...
            {view === 'performance' && (
                <div className="space-y-4">
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xl font-semibold text-[#1a1a1a]">Vendor Performance Scorecard</h3>
                            <div className="w-48">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">On-Time %</p>
                                <p className="text-3xl font-bold text-[#1a1a1a]">{formatPercentage(performance.vendor.metrics.onTimePercentage, 0)}</p>
                            </div>
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">Reliability Score</p>
                                <p className="text-3xl font-bold text-[#1a1a1a]">{performance.vendor.metrics.reliabilityScore.toFixed(0)}/100</p>
                            </div>
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">Ranking</p>
                                <p className="text-3xl font-bold text-[#1a1a1a]">#{performance.ranking} of {performance.totalVendors}</p>
                            </div>
                        </div>
                        ...
                        {performance.alternatives && (
                            <Card className="p-4 mt-4 bg-black/[0.03] border-black/[0.08]">
                                <h4 className="font-semibold mb-3 text-[#1a1a1a]">Alternative Vendors</h4>
                                <div className="space-y-2">
                                    {performance.alternatives.map((alt, idx) => (
                                        <div key={idx} className="p-3 bg-blue-500/10 rounded border border-blue-500/20">
                                            <p className="font-medium text-[#1a1a1a]">{alt.vendorName}</p>
            ...
            {view === 'quotes' && (
                <div className="space-y-4">
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <h3 className="text-xl font-semibold mb-4 text-[#1a1a1a]">Quote Follow-Up Tracker</h3>

                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">Open Rate</p>
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatPercentage(metrics.openRate)}</p>
                            </div>
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">Click Rate</p>
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatPercentage(metrics.clickRate)}</p>
                            </div>
                            <div className="p-4 bg-black/[0.03] rounded-lg border border-black/[0.04]">
                                <p className="text-sm text-gray-400">Reply Rate</p>
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatPercentage(metrics.replyRate)}</p>
                            </div>
                            <div className="p-4 bg-green-500/10 rounded-lg border border-green-500/20">
                                <p className="text-sm text-green-200">Conversion Rate</p>
                                <p className="text-2xl font-bold text-[#1a1a1a]">{formatPercentage(metrics.conversionRate)}</p>
                            </div>
                        </div>

                        <div className="space-y-3">
                            <h4 className="font-semibold text-[#1a1a1a]">Recent Quotes</h4>
                            {[quote, trackQuoteEngagement('quote-2'), trackQuoteEngagement('quote-3')].map((q, idx) => (
                                <Card key={idx} className="p-4 bg-black/[0.03] border-black/[0.08]">
                                    <div className="flex items-center justify-between mb-2">
                                        <div>
                                            <p className="font-medium text-[#1a1a1a]">{q.customerName}</p>
                                            <p className="text-sm text-gray-400">{q.quoteId}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-lg font-bold text-[#1a1a1a]">{formatCurrency(q.quotedAmount)}</p>
