...
import { generateMockShipments } from '../GasperTool/aiService';
...
export function ShipmentDashboard() {
        ...
        return (
            <div className="flex items-center justify-center h-full min-h-[400px]">
                <div className="flex flex-col items-center gap-4">
                    <div className="w-12 h-12 rounded-full border-4 border-black/[0.08] border-t-[#0EA5E9] animate-spin"></div>
                    <p className="text-gray-500 animate-pulse">Loading Shipments...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto pb-10">
            {/* Top Bar */}
            <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white/05 p-4 rounded-2xl border border-black/[0.08] backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto no-scrollbar">
                    <div className="flex bg-black/20 p-1 rounded-lg border border-black/[0.08] shrink-0">
                        <button
                            onClick={() => setViewMode('list')}
                            className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-2", viewMode === 'list' ? "bg-[#0EA5E9] text-[#1a1a1a] shadow-lg" : "text-gray-500 hover:text-[#1a1a1a]")}
                        >
                            <FileText size={14} /> List
                        </button>
                        <button
                            onClick={() => setViewMode('map')}
                            className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-2", viewMode === 'map' ? "bg-[#0EA5E9] text-[#1a1a1a] shadow-lg" : "text-gray-500 hover:text-[#1a1a1a]")}
                        >
                            <MapPin size={14} /> Map
                        </button>
                    </div>
                    <div className="h-6 w-px bg-black/[0.06] mx-2 hidden md:block"></div>
                        ...
                        {['all', 'in_transit', 'customs', 'delayed', 'delivered'].map((f) => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    filter === f
                                        ? "bg-black/[0.06] text-[#1a1a1a] border-black/[0.12]"
                                        : "bg-transparent text-gray-400 border-transparent hover:bg-black/[0.03] hover:text-[#1a1a1a]"
                                )}
                            >
                                {f.replace('_', ' ')}
                            </button>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="relative flex-1 md:w-48">
                        ...
                        <input
                            ...
                            className="w-full bg-black/20 border border-black/[0.08] rounded-xl pl-10 pr-4 py-2 text-sm text-[#1a1a1a] focus:outline-none focus:border-[#0EA5E9]/50 transition-colors"
                        />
                    </div>
                    <Button onClick={() => alert("This operational feature is currently in preview.")} className="bg-white text-black hover:bg-gray-200 rounded-xl shadow-lg shadow-white/10 gap-2 font-semibold whitespace-nowrap px-4">
            ...
            {/* Content Area */}
            {viewMode === 'map' ? (
                <div className="w-full h-[600px] rounded-2xl overflow-hidden border border-black/[0.08] shadow-2xl bg-black/20">
                    <ShipTrackingMap />
                </div>
            ) : (
                <>
                                ...
                                <div key={shipment.id} className="shipment-card group">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-black/[0.03] flex items-center justify-center border border-black/[0.08] group-hover:border-black/[0.12] transition-colors">
                                                <Package className="w-6 h-6 text-gray-500" />
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-3 mb-1">
                                                    <h3 className="text-lg font-bold text-[#1a1a1a]">{shipment.id}</h3>
                                                    <Badge variant={getStatusVariant(shipment.status)} className="white-10 backdrop-blur-sm">
                                                        {shipment.status.replace('_', ' ')}
                                                    </Badge>
                                                </div>
                                                <p className="text-sm text-gray-500">{shipment.carrier} • Air Freight • {shipment.weight || '500kg'}</p>
                                            </div>
                                        </div>
                                        <div className="text-left md:text-right">
                                            <p className="text-sm text-gray-400 mb-1">Estimated Arrival</p>
                                            <p className="text-xl font-mono text-[#1a1a1a] flex items-center gap-2 md:justify-end">
                                    ...
                                    <div className="shipment-progress-track">
                                        {/* Background Line */}
                                        <div className="absolute top-1/2 left-0 w-full h-0.5 bg-black/[0.06] -translate-y-1/2 z-0"></div>

                                        {/* Active Line (Mocked width based on status) */}
                                        <div
                                            className="absolute top-1/2 left-0 h-0.5 bg-[#0EA5E9] -translate-y-1/2 z-0 transition-all duration-1000"
                                            style={{
                                                ...
                                                <div key={index} className="relative z-10 flex flex-col items-center">
                                                    <div className={cn(
                                                        ...
                                                        status === 'completed' || status === 'active'
                                                            ? "border-[#0EA5E9] text-[#0EA5E9] shadow-[0_0_15px_rgba(82,39,255,0.4)]"
                                                            : "border-black/[0.08] text-gray-200"
                                                    )}>
                                                        <step.icon size={14} />
                                                    </div>
                                                    <span className={cn(
                                                        "absolute top-10 text-[10px] uppercase tracking-wider font-semibold whitespace-nowrap transition-colors",
                                                        status === 'active' ? "text-[#1a1a1a]" : "text-gray-300"
                                                    )}>{step.label}</span>
                                                </div>
                                            )
                                        })}
                                    ...
                                    {/* Footer Actions */}
                                    <div className="mt-8 pt-4 border-t border-black/[0.04] flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        ...
                                        <Button onClick={() => alert("This operational feature is currently in preview.")} size="sm" variant="ghost" className="text-gray-500 hover:text-[#1a1a1a] hover:bg-black/[0.06] group/btn">
