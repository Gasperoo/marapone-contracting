...
export function CommodityTicker() {
    ...
    const [commodities, setCommodities] = useState([
        ...
        { name: 'Natural Gas', symbol: 'NG', price: 2.85, change: 3.1, unit: 'USD/MMBtu', icon: Droplets, color: 'text-[#FF6B00]' },
    ]);
        ...
        <Card className="p-5 glass-panel border-0 bg-black/[0.03] relative overflow-hidden h-full">
            ...
            <div className="space-y-4">
                {commodities.map((item, index) => (
                    <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-black/[0.03] border border-black/[0.04] hover:border-black/[0.12] transition-all">
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-full bg-black/[0.03] ${item.color}`}>
                                <item.icon size={16} />
                            </div>
                            <div>
            ...
            <div className="mt-4 text-[10px] text-gray-300 text-center">
                Simulated Live Feed
            </div>
        </Card>
    );
