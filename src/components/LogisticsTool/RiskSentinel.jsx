...
} from '../GasperTool/services/riskMarketService';
        ...
        <div className="space-y-6 max-w-[1600px] mx-auto pb-12 px-4 md:px-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-400 text-sm">Global Supply Chain Threat Matrix</p>
                </div>
                ...
                <div className="flex-1 max-w-2xl bg-white border border-black/[0.08] rounded-full py-2 px-4 overflow-hidden relative">
                    <div className="flex items-center gap-3 animate-marquee whitespace-nowrap">
                        ...
                        {newsFeed.map((news, i) => (
                            <span key={i} className="text-xs text-gray-400 mx-4 flex items-center gap-2">
                                <span className={`w-1.5 h-1.5 rounded-full ${news.type === 'critical' ? 'bg-red-500 animate-pulse' : 'bg-yellow-500'}`}></span>
                                {news.message}
                                <span className="text-gray-300 text-[10px] ml-1">[{news.time}]</span>
                            </span>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-12 gap-6">
                        ...
                        {riskMap.map((point, i) => {
                            ...
                            return (
                                <div key={i}
                                    ...
                                    <div className={`w-6 h-6 rounded-full relative z-10 flex items-center justify-center border border-black/[0.12] shadow-[0_0_15px_rgba(239,68,68,0.5)]
                                        ${point.severity === 'critical' ? 'bg-red-500' :
                                            point.severity === 'high' ? 'bg-orange-500' : 'bg-yellow-600'}`}>
                                        <Icon size={12} className="text-[#1a1a1a]" />
                                    </div>
                                    ...
                                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-48 bg-black/90 border border-red-500/30 p-3 rounded backdrop-blur-md opacity-0 group-hover/point:opacity-100 transition-opacity z-20 pointer-events-none">
                                        ...
                                        <div className="text-[10px] text-gray-500 mb-1">{point.location}</div>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div className="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <Card className="bg-white border-black/[0.08] p-5">
                        <h3 className="text-sm font-semibold text-[#1a1a1a] flex items-center gap-2 mb-4 uppercase tracking-wider">
                            <Activity className="text-[#0EA5E9] w-4 h-4" />
                            AI Predictive Analysis
                        </h3>
                        <div className="space-y-4">
                            {riskScores.map((score, i) => (
                                <div key={i} className="bg-black/[0.03] border border-black/[0.04] p-3 rounded hover:bg-black/[0.06] transition-colors">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-sm font-medium text-[#1a1a1a]">{score.category}</span>
                    ...
                    <Card className="h-[260px] bg-white border-black/[0.08] p-5 flex flex-col">
                        <h3 className="text-sm font-semibold text-[#1a1a1a] flex items-center gap-2 mb-4 uppercase tracking-wider">
                            <GitPullRequest className="text-[#0EA5E9] w-4 h-4" />
                            Dependency Graph
                        </h3>
                        <div className="flex-1 overflow-y-auto relative scrollbar-hide">
                            {/* Visual Tree Representation */}
                            <div className="flex flex-col gap-4 pl-2 border-l border-black/[0.08] ml-2">
                                <div className="flex items-center gap-3">
                                    <div className="w-3 h-3 rounded-full bg-emerald-500 ring-4 ring-emerald-500/20"></div>
                                    <span className="text-sm font-bold text-[#1a1a1a]">Marapone HQ</span>
                                </div>

                                <div className="pl-6 space-y-4 border-l border-black/[0.08] ml-[5px]">
                                    {/* Sub-node Cluster 1 */}
                                    <div className="relative">
                                        <div className="absolute -left-[25px] top-3 w-6 hpx bg-black/[0.06]"></div>
                                        <div className="bg-black/[0.03] p-2 rounded border border-black/[0.04] flex justify-between items-center">
                                            <div className="flex items-center gap-2">
                                                <GitCommit size={14} className="text-orange-400" />
                                                <span className="text-xs text-[#1a1a1a]">Foxconn (CN)</span>
                                            </div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="bg-red-500/10 p-2 rounded border border-red-500/30 flex justify-between items-center shadow-[0_0_10px_rgba(239,68,68,0.1)]">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-[#1a1a1a] font-medium">TSMC (TW)</span>
                                            </div>
                                            <span className="text-[10px] text-red-400 font-mono font-bold">RISK: 85</span>
                                        </div>
                                        {/* Tier-2 Supplier */}
                                        <div className="pl-6 mt-2 relative">
                                            <div className="absolute -left-[14px] top-3 w-4 hpx bg-black/[0.06]"></div>
                                            <div className="bg-black/[0.03] p-1.5 rounded border border-black/[0.04] flex gap-2 w-fit">
                                                <Truck size={10} className="text-[#0EA5E9]" />
                                                <span className="text-[10px] text-gray-400">DHL Aviation</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="bg-black/[0.03] p-2 rounded border border-black/[0.04] flex justify-between items-center">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-[#1a1a1a]">Samsung (KR)</span>
                                            </div>
