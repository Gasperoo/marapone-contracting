...
} from '../GasperTool/services/complianceOperationsService';
                    ...
                    <Card className="h-[600px] bg-white border-black/[0.08] p-0 relative overflow-hidden group">
                        ...
                        <div className="absolute top-6 left-6">
                            <h3 className="text-lg font-semibold text-[#1a1a1a] flex items-center gap-2">
                                <Globe className="text-[#0EA5E9] w-5 h-5" />
                                Regulatory Radar
                            </h3>
                                    ...
                                    <div key={i} className="bg-black/[0.03] border border-black/[0.04] p-2 rounded flex items-center justify-between text-xs group hover:bg-black/[0.06] transition-colors">
                                        ...
                                        <div className="text-right">
                                            <div className={`px-2 py-0.5 rounded text-[10px] font-bold 
                                                ${log.status === 'PASS' || log.status === 'VERIFIED' || log.status === 'CLEARED' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-blue-500/10 text-[#0EA5E9]'}`}>
                                                {log.status}
                                            </div>
                        ...
                        <div className="absolute inset-0 p-6 flex flex-col z-10">
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-sm font-semibold text-[#1a1a1a] flex items-center gap-2 uppercase tracking-wider">
                                    <ScanLine className="text-[#0EA5E9] w-4 h-4" />
                                    AI Optical Scanner
                                </h3>
                                {scanning ? (
                                    <span className="text-xs text-[#0EA5E9] animate-pulse font-mono">SCANNING...</span>
                                ) : (
