...
import {
    ...
    getESGScorecard,
    getGreenCorridors
} from '../GasperTool/services/sustainabilityService';
        ...
        <div className="space-y-6 max-w-[1600px] mx-auto pb-12 px-4 md:px-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-400 text-sm">Autonomous ESG Control & Carbon Intelligence</p>
                </div>
                ...
                <div className="flex items-center gap-4 bg-emerald-500/10 border border-emerald-500/20 px-6 py-3 rounded-2xl">
                    <div>
                        ...
                        <p className="text-2xl font-black text-[#1a1a1a]">{scorecard.overall}<span className="text-emerald-500 text-sm">/100</span></p>
                    </div>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-8">
                    <Card className="h-[550px] bg-[#05140b] border-emerald-500/10 p-0 relative overflow-hidden group">
                        ...
                        {heatmapData.map((point, i) => {
                            ...
                            return (
                                <div key={i}
                                    ...
                                    <div className={`w-5 h-5 rounded-full relative z-10 flex items-center justify-center border border-black/[0.12]
                                        ${point.type === 'offset' ? 'bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.5)]'}`}>
                                        {point.type === 'offset' ? <TreePine size={10} className="text-[#1a1a1a]" /> : <Wind size={10} className="text-[#1a1a1a]" />}
                                    </div>
                                    ...
                                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-48 bg-black/90 border border-emerald-500/30 p-3 rounded backdrop-blur-md opacity-0 group-hover/point:opacity-100 transition-opacity z-20 pointer-events-none">
                                        ...
                                        <div className="text-[10px] text-gray-500">{point.status}</div>
                        ...
                        {/* Map HUD */}
                        <div className="absolute bottom-6 left-6 flex gap-6 bg-black/40 backdrop-blur-md p-4 rounded-xl border border-black/[0.04]">
                            <div>
                                <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Global Intensity</p>
                                <div className="h-2 w-32 bg-black/[0.03] rounded-full overflow-hidden flex">
                            ...
                            <div className="border-l border-black/[0.08] pl-6">
                                <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Active Projects</p>
                                <p className="text-lg font-bold text-[#1a1a1a]">12,482 <span className="text-[10px] text-emerald-400">↑ 12%</span></p>
                            </div>
                        </div>
                    </Card>
                </div>
                ...
                <div className="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <Card className="bg-[#05140b] border-emerald-500/10 p-5">
                        ...
                        <div className="space-y-4">
                            {scorecard.levers.map((lever, i) => (
                                <div key={i} className="bg-black/[0.03] p-3 rounded-xl border border-black/[0.04]">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <p className="text-[10px] text-gray-400 font-bold uppercase">{lever.label}</p>
                                            <p className="text-lg font-bold text-[#1a1a1a]">{lever.value.toLocaleString()} <span className="text-xs text-gray-400 font-normal">{lever.unit}</span></p>
                                        </div>
                                    ...
                                    <div className="h-1 w-full bg-black/[0.03] rounded-full mt-3">
                                        <div
                    ...
                    <Card className="h-[275px] bg-[#05140b] border-emerald-500/10 p-5 overflow-hidden flex flex-col">
                        <h3 className="text-sm font-semibold text-[#1a1a1a] flex items-center gap-2 mb-4 uppercase tracking-wider">
                            <Compass className="text-[#0EA5E9] w-4 h-4" />
                            Green Corridor Navigator
                        </h3>
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide">
                            {greenCorridors.map((route, i) => (
                                <div key={i} className="group cursor-pointer bg-black/[0.03] hover:bg-emerald-500/10 border border-black/[0.04] hover:border-emerald-500/30 p-3 rounded-xl transition-all">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-xs font-bold text-[#1a1a1a]">{route.route}</span>
                                        <span className="text-[10px] bg-blue-500/10 text-[#0EA5E9] px-2 py-0.5 rounded font-mono">-{route.reduction} CO2</span>
                                    </div>
                ...
                <Card className="col-span-12 bg-[#05140b] border-emerald-500/10 p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-gray-300 text-xs font-mono uppercase">Verified Environmental Assets • Immutable Audit Trail</p>
                            </div>
                        </div>
                        <Button onClick={() => alert("This operational feature is currently in preview.")} className="bg-emerald-600 hover:bg-emerald-500 text-[#1a1a1a] gap-2">
                            <Leaf size={16} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            ...
                            <div key={i} className="relative p-4 rounded-2xl bg-black/[0.03] border border-black/[0.04] hover:border-emerald-500/30 transition-all cursor-pointer overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="flex justify-between items-end">
                                        <div>
                                            <p className="text-[10px] text-gray-400 uppercase">Lifetime Impact</p>
                                            <p className="text-lg font-black text-[#1a1a1a]">{project.impact}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] text-gray-400 uppercase">Region</p>
                                            <p className="text-xs font-bold text-[#1a1a1a]">{project.region}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
