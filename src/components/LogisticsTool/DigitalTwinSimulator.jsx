...
import { runSupplyChainSimulation, simulateTariffChange } from '../GasperTool/services/simulationService';
    ...
    return (
        <div className="space-y-6 max-w-[1600px] mx-auto pb-12">
            ...
            <div className="grid grid-cols-12 gap-6 min-h-[600px] relative">
                ...
                {/* 3D Main Globe View */}
                <div className="col-span-12 lg:col-span-9 relative group overflow-hidden rounded-3xl border border-black/[0.08] bg-white shadow-2xl">
                    {/* Background Grid & Stars */}
                    <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:40px_40px] opacity-20"></div>

                    {/* The "Globe" (CSS 3D approximation) */}
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="w-[800px] h-[800px] rounded-full bg-[radial-gradient(circle_at_30%_30%,#1e293b,black)] opacity-80 border border-black/[0.04] shadow-[0_0_100px_rgba(59,130,246,0.1)] relative animate-slow-spin">
                            {/* Map Texture Overlay */}
                    ...
                    <div className="absolute top-6 left-6 flex gap-4">
                        <div className="bg-black/40 backdrop-blur-md border border-black/[0.08] px-4 py-2 rounded-full flex items-center gap-2">
                    ...
                    <div className="absolute bottom-6 left-6 right-6 md:right-auto md:w-80 bg-black/60 backdrop-blur-xl border border-black/[0.08] p-5 rounded-2xl shadow-xl z-20">
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="text-[10px] font-mono text-gray-500 uppercase">Event Type</label>
                                <Select value={scenarioType} onValueChange={setScenarioType}>
                                    <SelectTrigger className="h-8 bg-black/[0.03] border-black/[0.08] text-xs"><SelectValue /></SelectTrigger>
                                    <SelectContent>
                            ...
                            <div>
                                <label className="text-[10px] font-mono text-gray-500 uppercase">Severity</label>
                            ...
                            <Button
                                ...
                                disabled={isLoading}
                                className="w-full h-9 bg-red-600 hover:bg-red-700 text-[#1a1a1a] border border-red-500/50 shadow-[0_0_15px_rgba(220,38,38,0.4)]"
                            >
                ...
                <div className="col-span-12 lg:col-span-3 space-y-4 h-full flex flex-col">
                    {/* Financial Ticker */}
                    <Card className="bg-white border-black/[0.08] p-5 flex-1 flex flex-col justify-center items-center text-center relative overflow-hidden group">
                        <div className="absolute inset-0 bg-blue-500/5 group-hover:bg-blue-500/10 transition-colors"></div>
                        <DollarSign className="w-10 h-10 text-blue-500 mb-2 opacity-80" />
                        <div className="text-3xl font-mono font-bold text-[#1a1a1a] mb-1">
                            {formatCurrency(ticker.revenue).split('.')[0]}
                        </div>
                        <div className="text-xs font-mono text-[#0EA5E9] uppercase tracking-widest">Global Revenue Flow</div>
                    </Card>

                    {/* Risk Gauge */}
                    <Card className={`bg-white border-black/[0.08] p-5 flex-1 flex flex-col justify-center items-center text-center relative overflow-hidden transition-all duration-500 ${result ? 'border-red-500/50 bg-red-950/20' : ''}`}>
                        <AlertTriangle className={`w-10 h-10 mb-2 transition-colors ${result ? 'text-red-500 animate-pulse' : 'text-gray-200'}`} />
                        <div className={`text-3xl font-mono font-bold transition-colors ${result ? 'text-red-500' : 'text-gray-400'}`}>
                            {result ? formatCurrency(ticker.risk).split('.')[0] : '$0'}
                        </div>
                        <div className="text-xs font-mono text-gray-400 uppercase tracking-widest">Revenue at Risk</div>
                    </Card>

                    {/* AI Insight Feed */}
                    <Card className="bg-white border-black/[0.08] p-4 flex-[2] overflow-hidden flex flex-col">
                        ...
                        <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                            {result ? (
                                <>
                                    ...
                                    <div className="p-3 bg-black/[0.03] border border-black/[0.08] rounded-lg">
                                        <p className="text-[10px] text-gray-400 uppercase mb-1">Recommended Action</p>
                                        <p className="text-xs text-[#1a1a1a] leading-relaxed font-mono">
                                            {result.aiPrediction.nextBestAction}
                                        </p>
                                    </div>
                                    <div className="p-3 bg-black/[0.03] border border-black/[0.08] rounded-lg">
                                        <p className="text-[10px] text-gray-400 uppercase mb-1">Affected Nodes</p>
            ...
            {result && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-10 fade-in duration-700">
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-400">Projected Delay</span>
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-400">Efficiency Drop</span>
                    ...
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <h4 className="font-bold text-[#1a1a1a] mb-4 flex items-center gap-2">
                            <Shield size={18} className="text-[#0EA5E9]" /> Mitigation Strategy
                        </h4>
                    ...
                    <Card className="p-6 bg-black/[0.03] border-black/[0.08]">
                        <h4 className="font-bold text-[#1a1a1a] mb-4 flex items-center gap-2">
                            <Cpu size={18} className="text-[#0EA5E9]" /> AI Confidence
                        </h4>
                        <div className="flex flex-col items-center justify-center py-2">
                            <div className="relative w-32 h-32 flex items-center justify-center">
                                ...
                                <span className="absolute text-2xl font-bold text-[#1a1a1a]">{result.aiPrediction.confidence}%</span>
                            </div>
                            <p className="text-xs text-gray-400 mt-2 text-center max-w-[200px]">Based on analysis of 14,000+ similar historical events.</p>
                        </div>
                    </Card>
                </div>
            )}
