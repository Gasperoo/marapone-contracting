 ...
 * The same one rule applies to PACK_DOWNLOAD_URLS below, per SKU. Everything
...
/**
 * Per-SKU download links for the data packs. Same rule as DOWNLOAD_URL: a key
 * that is empty (or missing) degrades every surface to "sent by hand within
 * PACK_FULFILMENT_HOURS". Fill a key in and that one pack becomes an instant
 * link in the email and on the success page, with no other change anywhere.
 *
 * These are five ZIPs and a bundle ZIP — not licensed software — so they can
 * be filled in independently as each one is finished.
 */
export const PACK_DOWNLOAD_URLS = {
  'pack-code-library': '',
...
export const packDownloadUrl = (product) => PACK_DOWNLOAD_URLS[product] || '';
export const hasPackDownload = (product) => !!packDownloadUrl(product);
...
export function packHandoffEmail({ name, email, product }) {
  const f = packFulfilmentFor(product);
  if (!f) return null;
  const url = packDownloadUrl(product);
